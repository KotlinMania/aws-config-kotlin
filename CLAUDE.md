# Claude Code Project Instructions — aws-config-kotlin

## Project Overview

This is **aws-config-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`aws-config`](https://crates.io/crates/aws-config).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/aws-config/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/aws-config>
